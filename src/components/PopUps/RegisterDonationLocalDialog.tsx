...
export default function SignUpDialog() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const response = await fetch("http://localhost:3333/new-donation-local", {
      method: "POST",
      headers: {
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      <Dialog.Trigger asChild>
        <span className="w-full ml-3">Register Donation Local</span>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 w-full max-w-md z-50">
          ...
          <Dialog.Description className="text-sm mb-10 text-black/50">
            Please fill in the Donation Local address.
          </Dialog.Description>
